...
Команды:
  ...
  python convert.py singbox  — пересоздать sing-box rule-set (.json + .srs рядом с .lst)
...
def _singbox_rules(lst_file):
    """Правила sing-box из .lst: domain_suffix для доменов, ip_cidr для подсетей."""
    domains, ips = [], []
    with open(lst_file, encoding="utf-8") as f:
        for line in f:
            e = line.strip()
...
def to_singbox(lst_file, json_file, sb):
    """Рядом с .lst записать sing-box rule-set .json и, если доступен sing-box, .srs."""
    rules = _singbox_rules(lst_file)
    if not rules:
        return 0
    ...
    if sb:
        subprocess.run(
            [sb, "rule-set", "compile", "--output",
             json_file.replace(".json", ".srs"), json_file],
            check=True,
        )
    return sum(len(v) for r in rules for v in r.values())
...
def generate_singbox():
    ...
    if not sb:
        print("[singbox] sing-box не найден в PATH — только .json, без .srs")
    lst_files = sorted(glob.glob(os.path.join(BASE_DIR, "**", "*.lst"), recursive=True))
    n = 0
    for lst in lst_files:
        if any(p in lst for p in skip) or "-surge" in lst:
            continue
        to_singbox(lst, lst.replace(".lst", ".json"), sb)
        n += 1
    print(f"[singbox] Готово: {n} rule-set" + (" (.json + .srs)" if sb else " (.json)"))
